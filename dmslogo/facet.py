...
def facet_plot(
    ...
    *,
    ...
    for groupname, group in groups:
        ...
        for col in cols[2:]:
            if (len(firstgroup[col]) != len(group[col])) or any(
                firstgroup[col] != group[col]
            ):
                raise ValueError(
    ...
    possible_funcs = [
        ("draw_line", draw_line_kwargs, line_titlesuffix),
    ...
    if len(draw_funcs) < 1:
        raise ValueError(
            "set at least one of: "
            + ", ".join(tup[0] + "_kwargs" for tup in possible_funcs)
        )
    ...
    for name, name_d in draw_funcs.items():
        for colname, col in [("x_col", x_col), ("show_col", show_col)]:
            if (colname in name_d["kwargs"]) and (name_d["kwargs"][colname] != col):
                raise ValueError(
                    f"{colname} is in {name}_kwargs; "
        ...
        for (row, _), idata in name_d["data"].groupby([gridrow_col, gridcol_col]):
            ...
            for key, val in [
                ...
                if key not in name_d:
                    ...
                    raise ValueError(
                        f"inconsistent {key} for {name}: " f"{val} {name_d[key]}"
                    )
            ...
            for ltype, lfunc, val in [("min", min, ymin), ("max", max, ymax)]:
                if row not in fixed_ylims[ltype]:
                    fixed_ylims[ltype][row] = val
                else:
                    fixed_ylims[ltype][row] = lfunc(val, fixed_ylims[ltype][row])
    ...
    fig, axes = plt.subplots(
        ...
        gridspec_kw={
            "width_ratios": [
                d["width"] for d in draw_funcs.values() for _ in range(ncols_per_func)
            ]
        },
    )
    width = (
    ...
    axes_has_plot = _draw_facet_plots(
        axes, draw_funcs, ncols_per_func, gridrow_col, gridcol_col, nrows, fixed_ylims
    )
    fig.canvas.draw()
    ...
    if share_ylabel:
        ...
        for ifunc in range(nfuncs):
            _axes_to_centered_fig_label(
                fig, [axes[irow, ifunc * ncols_per_func] for irow in range(nrows)], "y"
            )
...
def _axes_to_centered_fig_label(fig, axlist, axistype):
    ...
    for propname, propset in list(label_props.items()):
        if len(propset) != 1:
            raise ValueError(f"multiple {propname} among axes: " f"{propset}")
...
def _draw_facet_plots(
    ...
    for ifunc, func_d in enumerate(draw_funcs.values()):
        groups = [
            (row_name, row_data)
            for row_name, row_data in func_d["data"].groupby(gridrow_col)
            if len(row_data)
        ]
        ...
        for irow, (row_name, row_data) in enumerate(groups):
            ...
            row_groups = [
                (col_name, col_data)
                for col_name, col_data in row_data.groupby(gridcol_col)
                if len(col_data)
            ]
